...
pygame.display.set_caption("touchball")
#=============
thread="image"
...
startimage=[400,pygame.image.load("start.png"),pygame.image.load("start.png").convert()]
#=============
while True:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if thread=="start":
                ...
                if ctrl[6].collidepoint(event.pos):
                    ...
                    rect=[]
                    for i in range(0,70*balldisplay[2] if balldisplay[2]<7 else 70*balldisplay[2]+70,70):
                        rect.append(pygame.Rect((10+i)//500*350,10+i%500,150,50))
                        rect.append(pygame.Rect((10+i)//500*350+150,10+i%500,150,50))
                    ctrl[8]=rect
                    for i in range(balldisplay[2] if balldisplay[2]<=7 else balldisplay[2]+1):
                        balldisplay[3].append("")
                        balldisplay[4].append(0)
            if thread=="kind":
                color[4]=[]
                for i in range(len(ctrl[8])):
                    color[4].append((100,100,100))
    ...
    if thread=="image":
        if startimage[0]>255:
            screen.blit(startimage[1],(0,0))
        if startimage[0]<=255 and startimage[0]>0:
            startimage[2].set_alpha(startimage[0])
            screen.blit(startimage[2],(0,0))
        if startimage[0]<=0 and startimage[0]>-300:
            screen.fill((70,70,70))
        if startimage[0]<=-200:
            thread="start"
        startimage[0]-=2
    #====================================
    if thread=="start":
        screen.blit(pygame.font.SysFont("arial",33).render("touchball experiment",True,(255,255,255)),(150,80))
        for i in range(4):
        ...
        for i in range(6):
            ...
            if i%2==1:
                screen.blit(myFont.render("↓",True,(255,255,255)),ctrl[i])
        for i in range(3):
            screen.blit(myFont.render(["balls:","speed:","kinds:"][i]+str(balldisplay[i]),True,(255,255,255)),(210,210+i*70))
        #
    ...
    if thread == "kind":
        ...
        for i in range(len(color[4])):
            ...
            try:
                if i%2==0:
                    screen.blit(myFont.render(balldisplay[3][round(i/2)],True,(255,255,255)),(ctrl[8][i].x+100,ctrl[8][i].y+10))
            except:
                None
            finally:
                if i%2==1:
                    screen.blit(myFont.render(str(balldisplay[4][round((i-1)/2)]),True,(255,255,255)),(ctrl[8][i].x+80,ctrl[8][i].y+10))
        for i in range(0,rnum,70):
            screen.blit(myFont.render("name:",True,(255,255,255)),((10+i)//500*350+10,20+i%500))
            screen.blit(myFont.render("num:",True,(255,255,255)),((10+i)//500*350+150,20+i%500))
        pygame.draw.rect(screen,(100,100,100),[350,10,300,50],0)
        screen.blit(myFont.render("start",True,(255,255,255)),(470,20))
    time.sleep(0.01)
